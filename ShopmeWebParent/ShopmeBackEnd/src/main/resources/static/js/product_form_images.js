...
function addNextExtraImageSection(index) {
	htmlExtraImage = `<div class="col border m-3 p-2" id="divExtraImage${index}">
				<div id="extraImageHeader${index}"><label>Extra Image #<span class= "add">${number + 1}</span>:</label></div>
				<div class= "m-2">
...
function updateCountNumber(){
	$(".divCount").each(function(index,element){
		element.innerHTML = "Extra Image #" + (index + 1) ;
		});
		
	}
	
function updateLabelRemove(){
	$(".add").each(function(index,element){
	numberLabel = parseInt((element.innerHTML));
	element.innerHTML= "" + (numberLabel-1);
