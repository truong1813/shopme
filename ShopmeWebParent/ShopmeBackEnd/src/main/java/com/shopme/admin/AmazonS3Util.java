...
public class AmazonS3Util {
	...
	public static List<String> listFolder(String folderName) {
		...
		ListObjectsRequest listRequest = ListObjectsRequest.builder().bucket(BUCKET_NAME)
				.prefix(folderName).build();
		
		ListObjectsResponse reponse = client.listObjects(listRequest);
		List<S3Object> contents  =reponse.contents();
	...
	public static void uploadFolder(String folderName, String fileName, InputStream inputStream) {
		S3Client client = S3Client.builder().build();
	...
	public static  void  removeFolder(String folderName) {
		...
		while (listIterator.hasNext()) {
			DeleteObjectRequest request = DeleteObjectRequest.builder().bucket(BUCKET_NAME)
					.key(listIterator.next().key()).build();
			client.deleteObject(request);
		}
	}
}
